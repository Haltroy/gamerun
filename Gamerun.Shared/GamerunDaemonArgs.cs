...
public class GamerunDaemonArgs
{
    ...
    public string ToBase64()
    {
        ...
        if (iGPUTreshold != 0)
        {
            ...
            {
                Tools.WriteVarUInt(memstream, iGPUTreshold);
            }
            else
            {
    ...
    public GamerunDaemonArgs(string base64)
    {
        ...
        var _igpuTresholdVLE = decoded[19];
        var _parkedCoresHasValue = decoded[20];
        var _parkedCoresVLE = decoded[21];
        var _pinnedCoresHasValue = decoded[22];
        var _pinnedCoresVLE = decoded[23];
        if (_nvMemHasValue)
        {
            if (_nvMemIsVLE)
            {
        ...
        if (_igpuTresholdHasValue)
        {
            if (_igpuTresholdVLE)
            {
                iGPUTreshold = Tools.DecodeVarUInt(memStream);
            }
            else
            {
                buffer = new byte[sizeof(uint)];
                bufferRead = memStream.Read(buffer);
                if (bufferRead != buffer.Length) throw new GamerunEndOfStreamException(memStream.Position);
                iGPUTreshold = BitConverter.ToUInt32(buffer);
            }
        }
    ...
    /// </summary>
    public uint iGPUTreshold { get; set; }

    /// <summary>
    ///     Determines if Nvidia's PowerMizer setting should be changed on Nvidia cards (with Nvidia's drivers, no change on
    ///     Nouveau and Nova).
    /// </summary>
